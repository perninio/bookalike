...
const Library = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      axios
        .get(dataserverAPIUserInteractionEndpoint)
        .then(result => {
          const {
            data: { data }
          } = result;
      ...
      <div>
        ...
        <div className="carousel">
          {ownbooks.length != 0 ? (
            ...
            >
              {ownbooks.map(res => {
                return <LibraryBook possesion_info={res} />;
              })}
            </Slider>
          ) : (
            <Alert color="warning">
        ...
        <div>
          ...
          <div>
            <div>
              <h3>Książki, którę chcę przeczytać:</h3>
            </div>
            <div className="carousel">
              {wantsbooks.length != 0 ? (
                <Slider
