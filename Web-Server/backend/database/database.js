...
async function deleterealtion(uid1, uid2) {
  return instance
    .cypher(
      "match (u1:User)-[r:friends]-(u2:User) where id(u1)=" +
        uid1 +
        " and id(u2)=" +
        uid2 +
        " delete r return u1"
...
async function checkstatusbetweenusers(uid1, uid2) {
  ...
  if (rel != 0) {
    var reqac = await instance
      .cypher(
        "match (u1:User)-[r:friends {relation:'accepted_request'}]-(u2:User) where id(u1)=" +
          uid1 +
          " and id(u2)=" +
          uid2 +
          " return type(r)"
    ...
    if (reqac == 0) {
      var reqse = await instance
        .cypher(
          "match (u1:User)-[r:friends {relation:'send_request'}]->(u2:User) where id(u1)=" +
            uid1 +
            " and id(u2)=" +
            uid2 +
            " return type(r)"
